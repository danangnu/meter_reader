...
def _classify_7seg_roi(roi_bw: np.ndarray) -> Tuple[str, float]:
    ...
    scores["2"] += 0.24*top_on + 0.42*bot_on_eff + 0.70*mid_on_eff \
                   + 0.65*max(0.0, mE - 0.22) - 0.30*mF - 0.25*mC \
...
def process(
            ...
            min_char_h: int,
            force_slots: typing.Optional[int],
            debug_slots: bool,
            dump_slots: bool,
            boost_contrast: bool,
            min_int_digits: typing.Optional[int],
            source: str = "auto",
            max_slots: int = MAX_REASONABLE_SLOTS_DEFAULT
           ) -> dict:
    if out_dir is None:
    ...
    # Cap obviously absurd auto slot counts (unless the user forced a value).
    # This prevents 20–30+ spurious slots caused by background text/glare.
    if not force_slots and max_slots and len(spans) > max_slots:
        spans = _merge_to(spans, proj, max_slots)
    digit_count = len(spans)

    # clamp for pathological autoslotting
    if max_slots is not None:
        digit_count = min(digit_count, max_slots)
        spans = spans[:digit_count]
    ...
    # EXTRA SAFETY: if auto slot count looks absurd but Tesseract found a plausible length,
    # prefer the Tesseract line read.
    absurd_slots = (not force_slots) and max_slots and (digit_count > (max_slots + ABSURD_SLOTS_MARGIN))
    if absurd_slots and tess_digits:
        if 3 <= len(tess_digits) <= max_slots:
            picked_src, picked_val = "tess", tess_clean
    ...
    if slot_full and (not tess_full or ham > 0) and not absurd_slots:
...
def main():
    ...
    ap.add_argument("--max-slots", type=int, default=MAX_REASONABLE_SLOTS_DEFAULT, help="Upper bound for auto-estimated digit slots (ignored if --force-slots). Default: 8")
    args = ap.parse_args()
